...
class Star(object):
    def __init__(self, x, y, star_type):
        ...
        self._frame = 0
        self._frame_incrementer = triangular()
        if self._star_type in [0, 1, 3, 4]:
            self._frame = randint(0, 2)
            self._max_frame = 2
        else:
            self._frame = randint(0, 4)
            self._max_frame = 4
...
class Stars(object):
    def __init__(self, resource_manager, position, amount=200):
        self._star_sprites = [resource_manager.get('star' + str(index)) for index in range(0, 12)]
        self._stars = [Star(randint(0, position[0]), randint(0, position[1]), randint(0, 5)) for i
                       in range(amount)]
...
class Credits(object):
    def __init__(self, font, font_dither):
        ...
        self._credits_sprites = [font.get(text, Screen.WINDOW_SIZE[0]) for text in self._credits_text]
        self._actual_text = 0
    ...
    def render(self, surface):
        ...
        for i in range(196, 196 + Screen.WINDOW_SIZE[0], 8):
            surface.blit(self._font_dither, (i, Screen.WINDOW_SIZE[1] - self._font_dither.get_height()))
...
class IntroScene(Scene):
    def __init__(self, context, name='intro', scene_speed=15):
        ...
        self._title_sprites = [context.resource_manager.get('title' + str(index)) for index in range(0, 5)]
        ...
        self._intro_text.insert(1, self._font_white.get(
            _("...the world's most powerful nuclear plant is going to blow!!!"), 256))
        self._credits = Credits(self._font_blue, self._font_dither)
        self._background = pygame.Surface((self._menu_image.get_width(), self._menu_image.get_height())).convert()
        self.get_menu()
        self._particles_manager = ParticlesManager()
        smoke = SmokeParticles(context.resource_manager, 'smoke')
    ...
    def _run_control_start(self):
        ...
        if not self._menu_group.visible:
            ...
            self._credits.text_y = Screen.WINDOW_SIZE[1]
    ...
    def run(self):
        ...
        if self._control.on(Control.START):
    ...
    def render(self, display):
        ...
        if 0 < self._title_frame < 12:
            tmp_surface = pygame.Surface((self._title_sprites[0].get_width(), self._title_frame)).convert_alpha()
            tmp_surface.fill((0, 0, 0, 0))
            pygame.transform.scale(self._title_sprites[0], (self._title_sprites[0].get_width(), self._title_frame),
                                   tmp_surface)
            self._background.blit(tmp_surface, (325 - tmp_surface.get_width() / 2, 22 - tmp_surface.get_height() / 2))
        ...
        display.virtual_screen.blit(self._background, (0, 0),
                                    (self._background_x_position, 0, display.WINDOW_SIZE[0], display.WINDOW_SIZE[1]))

        if self._background_x_position < 98:
            display.virtual_screen.blit(self._intro_text[0], (128 - self._intro_text[0].get_width() / 2, 68))
        elif self._background_x_position in list(range(98, 196)):
            display.virtual_screen.blit(self._intro_text[1], (128 - self._intro_text[1].get_width() / 2, 68))
        if not self._menu_group.visible:
            display.virtual_screen.blit(self._skip_text, (128 - self._skip_text.get_width() / 2, 176))
