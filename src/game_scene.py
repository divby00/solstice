...
class GameScene(scene.Scene):
    ...
    def _check_player_is_in_elevator(self):
        return self._player._x + 8 >= self._exit_point[0] \
               and self._player._x - 8 <= self._exit_point[0] + self._exit_point[2] \
               and self._player._y - 8 <= self._exit_point[1] + self._exit_point[3] \
               and self._player._y + 8 >= self._exit_point[1]
    ...
    def _run_rails(self):
        ...
        if rail_direction != 0:
            if not self._player.check_right_collision(self._current_level) and rail_direction == 1:
                self._player._x += self._renderer_object.speed[0]
    ...
    def _run_item_counter(self):
        if self._player._get_item_counter < 5:
            self._player._get_item_counter += 1

            if self._player._get_item_counter == 5:
                self._player._get_item_available = True

    def _run_recovery_counter(self):
        if not self._player._recovery_mode and self._player._life < 100:

            if self._player._recovery_counter < 100:
                self._player._recovery_counter += 1

            if self._player._recovery_counter == 100:
                self._player._recovery_mode = True

    def _run_control_right(self):
        self._player._recovery_counter = 0
        self._player._recovery_mode = False
    ...
    def _run_control_down(self):
        ...
        if self._player._get_item_available:
            x = self._player._x
            y = self._player._y
            ...
            for i in self._items:
                if x + 8 >= i.x \
                    ...
                    self._player._get_item_available = False
                    self._player._get_item_counter = 0
                    self._sound_player.play_sample('accept')

                    if not self._player._selected_item:
                        self._player._selected_item = i
                        self._items.remove(i)
                        self._renderer_object.change_animation((i.x, i.y), None)
                    else:
                        tmp_item = self._player._selected_item
                        x, y = i.x, i.y
                        self._player._selected_item = i
                        self._items.remove(i)
    ...
    def _run_control_action1(self):
        ...
        if self._player._shoot_avail and self._player._bullets > 0:
            self._player._recovery_counter = 0
            self._player._recovery_mode = False
            self._sound_player.play_sample('laser')
            self._player._firing = True
            self._player._bullets -= .3

    def _run_control_action2(self):
        self._player._recovery_counter = 0
        self._player._recovery_mode = False

        if self._player._selected_item is not None:
            self._player._using_item = True
            self._sound_player.play_sample('accept')
    ...
    def run(self):
        ...
        if self._menu_group.visible:
            ...
            self._control.event_driven = False
            if not self._player._dying:
                self._player._flying = False
                ...
                if self._control.on(control.Control.UP) and self._player._thrust > 0:
                    self._run_control_up()

                if self._control.on(control.Control.DOWN):
                    self._run_control_down()

                if self._control.on(control.Control.ACTION1) and not self._player._teleporting:
                    self._run_control_action1()

                if self._control.on(control.Control.ACTION2) and not self._player._teleporting:
