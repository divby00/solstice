...
class Enemy(object):
    ...
    def _get_left_collision_list(self):
        positions = [
            ((self._x - self._speed) / self._level.map.tilewidth, self._y / self._level.map.tileheight),
    ...
    def _get_right_collision_list(self):
        positions = [((self._x + 15 + self._speed) / self._level.map.tilewidth,
    ...
    def _get_upper_collision_list(self):
        positions = [
            (self._x / self._level.map.tilewidth, (self._y - self._speed) / self._level.map.tileheight),
    ...
    def _get_bottom_collision_list(self):
        positions = []
        positions.append((self._x / self._level.map.tilewidth,
    ...
    def _hits_player(self):
        ...
        if not player.dying and not player.inmortal:
            if (self._x + self._size[0] + 256) >= (player.x - 8) \
                ...
                player_crap_particles = PlayerUtils.get_particles_manager(self._game_context).get('crap')
        ...
        if not self._active:
            ...
            if self._respawn:
                if self._animation_respawn.counter < 2:
                    ...
                    if self._animation_respawn.active_frame < len(self._animation_respawn.frames) - 1:
        ...
        if self._active:
            if self._shock_counter > 0:
                ...
                screen.blit(self._animation.images[str(self._animation.active_frame)],
                            (self._x + 256 + self._animation.frames[self._animation.active_frame].offset_x,
                             self._y + 144 + self._animation.frames[self._animation.active_frame].offset_y))
